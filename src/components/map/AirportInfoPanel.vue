...
import type {
  MapAirportDetail,
  MapAirportDetailGate,
  MapAirportDetailRunway,
  MapAirportDetailHelipad,
} from '@/types/map'
...
const tabs = computed<Array<{ key: TabKey; label: string; count?: number }>>(() => {
  ...
  return [
    ...
    {
      key: 'start',
      label: t('map.startPosition'),
      count: (d.gates?.length || 0) + (d.runways?.length || 0) + (d.helipads?.length || 0),
    },
  ]
})
...
const filteredGates = computed(() => {
  ...
  return props.detail.gates.filter(
    (g) =>
...
const runwayPairs = computed(() => {
  ...
  return props.detail.runways.map((rwy) => ({
    ...
    surface: rwy.surfaceType || t('common.unknown'),
    widthM: rwy.widthM || 0,
  }))
})
...
<template>
  <div
    class="rounded-xl border border-gray-200/50 dark:border-gray-700/70 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-xl overflow-hidden"
  >
    <!-- Header -->
    <div
      class="flex items-center justify-between px-3 py-2 border-b border-gray-200/50 dark:border-gray-700/70 cursor-pointer"
      @click="emit('toggle-collapse')"
    >
      <div v-if="detail" class="min-w-0 flex-1">
        <div class="flex items-baseline gap-2">
          <span class="font-mono text-base text-blue-600 dark:text-blue-300">{{
            detail.icao
          }}</span>
          <span class="text-[10px] text-gray-500">{{ detail.airportType }}</span>
          <span v-if="detail.isCustom" class="text-[10px] text-emerald-400">{{
            t('map.custom')
          }}</span>
        </div>
      ...
      <svg
        class="h-4 w-4 text-gray-500 transition-transform shrink-0 ml-2"
    ...
    <div v-if="!collapsed && detail" class="max-h-[60vh] overflow-y-auto">
      ...
      <div
        v-if="activeTab === 'info'"
        class="p-3 space-y-2 text-xs text-gray-600 dark:text-gray-300"
      >
        <!-- Stats summary -->
        <div class="text-[11px] text-gray-400 flex flex-wrap gap-x-2">
          <span>{{ t('map.stats.runways') }}: {{ detail.runways.length }}</span>
          <span>{{ t('map.stats.gates') }}: {{ detail.gates?.length || 0 }}</span>
          <span>{{ t('map.stats.helipads') }}: {{ detail.helipads?.length || 0 }}</span>
          <span>{{ t('map.stats.taxiways') }}: {{ detail.taxiways?.length || 0 }}</span>
        </div>
        ...
        <div class="space-y-1">
          ...
          <div
            class="font-mono text-[11px] leading-relaxed text-gray-700 dark:text-gray-200 break-all"
          >
            {{ metarText || t('map.noMetar') }}
          </div>
          <div class="text-[10px] text-gray-500 uppercase tracking-wide mt-1">TAF</div>
          <div class="font-mono text-[11px] leading-relaxed text-gray-400 break-all">
            {{ tafText || t('map.noTaf') }}
          </div>
        </div>
        ...
        <div v-if="detail.runways.length > 0">
          ...
          <div class="space-y-0.5">
            ...
            >
              <span class="font-mono text-blue-600 dark:text-blue-200"
                >{{ rwy.end1Name || '?' }}/{{ rwy.end2Name || '?' }}</span
              >
              <div class="flex items-center gap-2 text-[10px] text-gray-400">
                <span v-if="rwy.widthM">{{ Math.round(rwy.widthM) }}m</span>
                <span>{{ rwy.surfaceType || t('common.unknown') }}</span>
      ...
      <div v-if="activeTab === 'start'" class="p-2">
        ...
        <div class="flex gap-1 mb-2">
          ...
          >
            {{
              st === 'gates'
                ? `${t('map.stats.gates')} (${detail.gates?.length || 0})`
                : st === 'runways'
                  ? `${t('map.stats.runways')} (${detail.runways.length})`
                  : `${t('map.stats.helipads')} (${detail.helipads?.length || 0})`
            }}
          </button>
        </div>
        ...
        <template v-if="startSubTab === 'gates'">
          ...
          <div class="max-h-52 space-y-0.5 overflow-y-auto pr-1">
            <button
              ...
              :class="
                selectedStartType === 'gate' && selectedStartIndex === idx
            ...
            >
              <div class="min-w-0">
                ...
                <div v-if="gate.airlines?.length" class="text-[9px] text-gray-500 truncate">
                  {{ gate.airlines.join(', ') }}
                </div>
              </div>
              <div class="flex items-center gap-1 shrink-0">
                <span
                  v-if="gateWidthLabel(gate.widthCode)"
                  class="rounded bg-gray-200 dark:bg-slate-700 px-1 py-0.5 text-[9px] text-gray-600 dark:text-gray-300"
                  >{{ gateWidthLabel(gate.widthCode) }}</span
                >
                <span
                  v-if="gateOpLabel(gate.operationType)"
                  class="rounded bg-gray-200 dark:bg-slate-700 px-1 py-0.5 text-[9px] text-gray-600 dark:text-gray-300"
                  >{{ gateOpLabel(gate.operationType) }}</span
                >
              </div>
            </button>
          </div>
          <div v-if="filteredGates.length === 0" class="text-[11px] text-gray-500 text-center py-2">
            {{ t('map.noGatesFound') }}
          </div>
        </template>
          ...
          <div class="space-y-1">
            ...
            >
              <div class="text-[10px] text-gray-400 mb-1">
                {{ pair.surface }} · {{ Math.round(pair.widthM) }}m
              </div>
              <div class="grid grid-cols-2 gap-1">
                <button
                  class="rounded px-2 py-1.5 text-center font-mono text-[11px] transition-colors"
                  :class="
                    selectedStartType === 'rwy-end1' && selectedStartIndex === rIdx
                      ? 'bg-emerald-100 dark:bg-emerald-900/40 border border-emerald-500/40 text-emerald-700 dark:text-emerald-200'
                ...
                <button
                  class="rounded px-2 py-1.5 text-center font-mono text-[11px] transition-colors"
                  :class="
                    selectedStartType === 'rwy-end2' && selectedStartIndex === rIdx
                      ? 'bg-emerald-100 dark:bg-emerald-900/40 border border-emerald-500/40 text-emerald-700 dark:text-emerald-200'
                      : 'bg-gray-200 dark:bg-slate-700/70 hover:bg-gray-300 dark:hover:bg-slate-600/70 text-blue-600 dark:text-blue-200'
                  "
        ...
        <template v-if="startSubTab === 'helipads'">
          <div class="space-y-0.5">
            <button
              ...
              class="flex w-full items-center justify-between rounded px-2 py-1.5 text-left transition-colors"
              :class="
                selectedStartType === 'helipad' && selectedStartIndex === idx
                  ? 'bg-emerald-100 dark:bg-emerald-900/40 border border-emerald-500/40 text-emerald-700 dark:text-emerald-200'
                  : 'bg-gray-100 dark:bg-slate-800/60 hover:bg-gray-200 dark:hover:bg-slate-700/60 text-gray-700 dark:text-gray-300'
              "
