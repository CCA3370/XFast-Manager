...
<template>
  ...
  >
    <div
      class="w-[900px] max-w-[95vw] max-h-[85vh] rounded-xl border border-gray-200/50 dark:border-gray-700/70 bg-white dark:bg-slate-900 shadow-2xl flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div
      ...
      >
        <div class="flex items-center gap-3">
          <h2 class="text-lg text-gray-900 dark:text-gray-100">{{ t('map.ofp.title') }}</h2>
          <template v-if="ofp">
            <span class="font-mono text-sm text-blue-600 dark:text-blue-300">{{
              ofp.callsign || ofp.flightNumber
            }}</span>
        ...
        <div v-if="!ofp" class="flex-1 flex items-center justify-center py-12">
          <div class="text-center text-gray-500">
            <div class="text-lg mb-2">{{ t('map.ofp.noPlanTitle') }}</div>
            <div class="text-sm">{{ t('map.ofp.noPlanDesc') }}</div>
          </div>
        </div>
        ...
        <template v-else>
          <!-- Quick stats bar -->
          <div
            class="flex items-center gap-4 border-b border-gray-200/50 dark:border-gray-700/70 px-4 py-2 text-[11px] text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-800/50"
          >
            <div>
              <span class="text-gray-500">ETE</span>
              <span class="ml-1 font-mono text-gray-700 dark:text-gray-200">{{
                ofp.ete || '-'
              }}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span class="ml-1 font-mono text-gray-700 dark:text-gray-200"
                >{{ Math.round(ofp.distance) }} nm</span
              >
            </div>
          ...
          <div class="flex border-b border-gray-200/50 dark:border-gray-700/70 px-4">
            ...
            >
              {{
                tab === 'flight'
                  ? t('map.ofp.tabs.flight')
                  : tab === 'navlog'
                    ? t('map.ofp.tabs.navlog')
                    : tab === 'performance'
                      ? t('map.ofp.tabs.performance')
                      : tab === 'fuel'
                        ? t('map.ofp.tabs.fuel')
                        : t('map.ofp.tabs.weights')
              }}
            </button>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-4">
            ...
            <div v-if="activeTab === 'flight'" class="space-y-4">
              <!-- Route -->
              <div
                class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
              >
                <div class="text-[11px] text-gray-500 mb-1">{{ t('map.ofp.route') }}</div>
                <div
                  class="font-mono text-[11px] text-gray-700 dark:text-gray-200 leading-relaxed break-all"
                >
                  {{ ofp.route || '-' }}
                </div>
              </div>
              ...
              <div
                class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
              >
                <div class="text-[11px] text-gray-500 mb-2">{{ t('map.ofp.verticalProfile') }}</div>
                <svg
                ...
                >
                  ...
                  <template v-if="tocX > 0">
                    <line
                      :x1="tocX"
                      :x2="tocX"
                      :y1="profilePadding.top"
                      :y2="profileSvgHeight - profilePadding.bottom"
                      stroke="#22c55e"
                      stroke-width="1"
                ...
                <div class="flex items-center gap-4 mt-2 text-[10px] text-gray-500">
                  ...
                  <span class="flex items-center gap-1">
                    <span
                      class="inline-block w-3 h-0.5 bg-amber-500"
                      style="border-top: 1px dashed"
                    ></span>
                    T/D
                  </span>
                  <span class="ml-auto"
                    >{{ t('map.ofp.total') }}: {{ Math.round(totalDistance) }} nm</span
                  >
                </div>
              </div>
              <!-- Summary cards -->
              <div class="grid grid-cols-3 gap-3">
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
                >
                  <div class="text-[11px] text-gray-500 mb-1">{{ t('map.ofp.fuelSummary') }}</div>
                  <div class="text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ fmtWeight(ofp.blockFuel) }} {{ wUnit }}
                  </div>
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.blockFuel') }}</div>
                  <div class="mt-1 text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ fmtWeight(ofp.tripFuel) }} {{ wUnit }}
                  </div>
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.tripFuel') }}</div>
                </div>
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
                >
                  <div class="text-[11px] text-gray-500 mb-1">{{ t('map.ofp.weights') }}</div>
                  <div class="text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ fmtWeight(ofp.zfw) }} {{ wUnit }}
                  </div>
                  <div class="text-[10px] text-gray-500">ZFW</div>
                  <div class="mt-1 text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ fmtWeight(ofp.tow) }} {{ wUnit }}
                  </div>
                ...
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
                >
                  <div class="text-[11px] text-gray-500 mb-1">{{ t('map.ofp.runways') }}</div>
                  <div class="text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ ofp.takeoffRunway || '-' }}
                  </div>
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.takeoffRunway') }}</div>
                  <div class="mt-1 text-sm font-mono text-gray-700 dark:text-gray-200">
                    {{ ofp.landingRunway || '-' }}
                  </div>
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.landingRunway') }}</div>
                </div>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'navlog'">
              <!-- Header row -->
              <div
                class="grid grid-cols-[1fr_80px_90px_80px_80px_80px_40px] gap-1 text-[10px] text-gray-500 border-b border-gray-200/50 dark:border-gray-700/70 pb-1 mb-1 px-1"
              >
                <div>{{ t('map.ofp.fix') }}</div>
                <div class="text-right">{{ t('map.ofp.altitude') }}</div>
                <div class="text-right">{{ t('map.ofp.wind') }}</div>
                <div class="text-right">ETA</div>
                <div class="text-right">{{ t('map.ofp.distance') }}</div>
                <div class="text-right">{{ t('map.ofp.fuelRemaining') }}</div>
                <div></div>
              </div>
                ...
                <div v-for="(wp, idx) in ofp.waypoints" :key="idx">
                  ...
                  >
                    <div class="flex items-center gap-1.5 min-w-0">
                      ...
                      <span
                        v-if="idx === tocIndex"
                        class="rounded bg-emerald-900/50 px-1 py-0 text-[9px] text-emerald-300 shrink-0"
                        >T/C</span
                      >
                      <span
                        v-if="idx === todIndex"
                        class="rounded bg-amber-900/50 px-1 py-0 text-[9px] text-amber-300 shrink-0"
                        >T/D</span
                      >
                      <span
                        :class="stageBg(wp.stage)"
                        class="rounded px-1 py-0 text-[9px] shrink-0"
                        >{{ wp.stage }}</span
                      >
                    </div>
                    <div class="text-right font-mono" :class="stageColor(wp.stage)">
                      {{ fmtAlt(wp.altitude) }}
                    </div>
                    <div class="text-right font-mono text-gray-400">{{ windComponent(wp) }}</div>
                    <div class="text-right font-mono text-gray-600 dark:text-gray-300">
                      {{ fmtTime(wp.timeFromDep) }}
                    </div>
                    <div class="text-right font-mono text-gray-400">
                      {{ Math.round(wp.distFromDep) }} nm
                    </div>
                    <div class="text-right font-mono text-cyan-300">
                      {{ fmtFuel(wp.fuelRemaining) }}
                    </div>
                    <div class="text-center text-gray-500">
                      <svg
                        class="h-3 w-3 inline transition-transform"
                        :class="expandedFixes.has(idx) ? 'rotate-180' : ''"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path
                  ...
                  <!-- Expanded details -->
                  <div
                    v-if="expandedFixes.has(idx)"
                    class="grid grid-cols-4 gap-2 px-3 py-2 bg-gray-100 dark:bg-slate-800/80 rounded-b text-[10px] mb-1"
                  >
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.position') }}</span>
                      <div class="font-mono text-gray-600 dark:text-gray-300">
                        {{ wp.latitude.toFixed(4) }}°, {{ wp.longitude.toFixed(4) }}°
                      </div>
                    </div>
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.oat') }}</span>
                      <div class="font-mono text-gray-600 dark:text-gray-300">{{ wp.oat }}°C</div>
                    </div>
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.legDistance') }}</span>
                      <div class="font-mono text-gray-600 dark:text-gray-300">
                        {{ Math.round(wp.distToNext) }} nm
                      </div>
                    </div>
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.fuelUsed') }}</span>
                      <div class="font-mono text-gray-600 dark:text-gray-300">
                        {{ fmtFuel(wp.fuelUsed) }}
                      </div>
                    </div>
              ...
              <!-- Footer -->
              <div
                class="flex items-center justify-between border-t border-gray-200/50 dark:border-gray-700/70 pt-2 mt-2 text-[10px] text-gray-500"
              >
                <span>{{ t('map.ofp.waypointsCount', { count: ofp.waypoints.length }) }}</span>
                <span>{{ t('map.ofp.total') }}: {{ Math.round(totalDistance) }} nm</span>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'performance'" class="space-y-4">
              <div class="grid grid-cols-2 gap-3">
                ...
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
                >
                  <div class="text-[11px] text-gray-500 mb-2">{{ t('map.ofp.takeoff') }}</div>
                  <div class="text-lg font-mono text-gray-700 dark:text-gray-200 mb-1">
                    {{ ofp.departure }} / {{ ofp.takeoffRunway || '-' }}
                  </div>
                  <div class="grid grid-cols-2 gap-2 text-[11px] mt-3">
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.altitude') }}</span>
                      <div class="font-mono text-gray-700 dark:text-gray-200">
                        {{ fmtAlt(ofp.cruiseAltitude) }}
                      </div>
                    </div>
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.costIndex') }}</span>
                      <div class="font-mono text-gray-700 dark:text-gray-200">
                        {{ ofp.costIndex }}
                      </div>
                    </div>
                ...
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
                >
                  <div class="text-[11px] text-gray-500 mb-2">{{ t('map.ofp.landing') }}</div>
                  <div class="text-lg font-mono text-gray-700 dark:text-gray-200 mb-1">
                    {{ ofp.arrival }} / {{ ofp.landingRunway || '-' }}
                  </div>
                  <div class="grid grid-cols-2 gap-2 text-[11px] mt-3">
                    <div>
                      <span class="text-gray-500">{{ t('map.ofp.landingWeight') }}</span>
                      <div class="font-mono text-gray-700 dark:text-gray-200">
                        {{ fmtWeight(ofp.lw) }} {{ wUnit }}
                      </div>
                    </div>
                    <div v-if="ofp.alternate">
                      <span class="text-gray-500">{{ t('map.ofp.alternate') }}</span>
                      <div class="font-mono text-gray-700 dark:text-gray-200">
                        {{ ofp.alternate }}
                      </div>
                    </div>
              ...
              >
                <div class="text-[11px] text-gray-500 mb-2">
                  {{ t('map.ofp.cruisePerformance') }}
                </div>
                <div class="grid grid-cols-4 gap-3 text-[11px]">
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.initialFlightLevel') }}</span>
                    <div class="font-mono text-lg text-blue-300">
                      {{
                        ofp.cruiseAltitude >= 10000
                          ? `FL${Math.round(ofp.cruiseAltitude / 100)}`
                          : ofp.cruiseAltitude
                  ...
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.costIndex') }}</span>
                    <div class="font-mono text-lg text-gray-700 dark:text-gray-200">
                      {{ ofp.costIndex }}
                    </div>
                  </div>
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.cruiseMach') }}</span>
                    <div class="font-mono text-lg text-gray-700 dark:text-gray-200">
                      {{ ofp.cruiseMach || '-' }}
                    </div>
                  </div>
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.distance') }}</span>
                    <div class="font-mono text-lg text-gray-700 dark:text-gray-200">
                      {{ Math.round(ofp.distance) }} nm
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-if="activeTab === 'fuel'" class="space-y-4">
              <div
                class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
              >
                <div class="text-[11px] text-gray-500 mb-3">{{ t('map.ofp.fuelBreakdown') }}</div>
                <div class="space-y-2.5">
                  <div
                    v-for="item in fuelBreakdown"
                    :key="item.label"
                    class="flex items-center gap-3"
                  >
                    <span class="w-24 text-[11px] text-gray-400 truncate">{{ item.label }}</span>
                    <div
                      class="flex-1 h-4 rounded-full bg-gray-200 dark:bg-slate-700/80 overflow-hidden"
                    >
                      <div
                    ...
                    <span
                      class="w-20 text-right font-mono text-[11px] text-gray-700 dark:text-gray-200"
                      >{{ fmtWeight(item.value) }} {{ wUnit }}</span
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-3 gap-3">
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3 text-center"
                >
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.blockFuel') }}</div>
                ...
                <div
                  class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3 text-center"
                >
                  <div class="text-[10px] text-gray-500">{{ t('map.ofp.reserveAlternate') }}</div>
                  <div class="text-lg font-mono text-amber-300">
                    {{ fmtWeight(ofp.reserveFuel + ofp.alternateFuel) }}
                  </div>
                  <div class="text-[10px] text-gray-500">{{ wUnit }}</div>
                </div>
              </div>
            ...
            <div v-if="activeTab === 'weights'" class="space-y-4">
              <div
                class="rounded-lg border border-gray-200/50 dark:border-gray-700/70 bg-gray-50 dark:bg-slate-800/60 p-3"
              >
                <div class="text-[11px] text-gray-500 mb-3">{{ t('map.ofp.weightSummary') }}</div>
                <div class="space-y-3">
                  <!-- ZFW -->
                  <div>
                    <div class="flex items-center justify-between text-[11px] mb-1">
                      <span class="text-gray-400">{{ t('map.ofp.zeroFuelWeight') }}</span>
                      <span class="font-mono text-gray-700 dark:text-gray-200"
                        >{{ fmtWeight(ofp.zfw) }} {{ wUnit }}</span
                      >
                    </div>
                    <div class="h-2 rounded-full bg-gray-200 dark:bg-slate-700/80 overflow-hidden">
                      <div
                        class="h-full rounded-full bg-emerald-500"
                        :style="{ width: ofp.tow > 0 ? `${(ofp.zfw / ofp.tow) * 100}%` : '0%' }"
                      ></div>
                    </div>
                  </div>
                  <!-- TOW -->
                  <div>
                    <div class="flex items-center justify-between text-[11px] mb-1">
                      <span class="text-gray-400">{{ t('map.ofp.takeoffWeight') }}</span>
                      <span class="font-mono text-gray-700 dark:text-gray-200"
                        >{{ fmtWeight(ofp.tow) }} {{ wUnit }}</span
                      >
                    </div>
                    <div class="h-2 rounded-full bg-gray-200 dark:bg-slate-700/80 overflow-hidden">
                  ...
                  <div>
                    <div class="flex items-center justify-between text-[11px] mb-1">
                      <span class="text-gray-400">{{ t('map.ofp.landingWeight') }}</span>
                      <span class="font-mono text-gray-700 dark:text-gray-200"
                        >{{ fmtWeight(ofp.lw) }} {{ wUnit }}</span
                      >
                    </div>
                    <div class="h-2 rounded-full bg-gray-200 dark:bg-slate-700/80 overflow-hidden">
                      <div
                        class="h-full rounded-full bg-purple-500"
                        :style="{ width: ofp.tow > 0 ? `${(ofp.lw / ofp.tow) * 100}%` : '0%' }"
                      ></div>
                    </div>
                  </div>
                </div>
              ...
              >
                <div class="text-[11px] text-gray-500 mb-2">
                  {{ t('map.ofp.weightComposition') }}
                </div>
                <div class="grid grid-cols-2 gap-3 text-[11px]">
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.payloadComponent') }}</span>
                    <div class="font-mono text-gray-700 dark:text-gray-200">
                      {{ fmtWeight(Math.max(ofp.tow - ofp.zfw - ofp.blockFuel, 0)) }} {{ wUnit }}
                    </div>
                  </div>
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.blockFuel') }}</span>
                    <div class="font-mono text-gray-700 dark:text-gray-200">
                      {{ fmtWeight(ofp.blockFuel) }} {{ wUnit }}
                    </div>
                  </div>
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.tripFuel') }}</span>
                    <div class="font-mono text-gray-700 dark:text-gray-200">
                      {{ fmtWeight(ofp.tripFuel) }} {{ wUnit }}
                    </div>
                  </div>
                  <div>
                    <span class="text-gray-500">{{ t('map.ofp.weightAtLanding') }}</span>
                    <div class="font-mono text-gray-700 dark:text-gray-200">
                      {{ fmtWeight(ofp.lw) }} {{ wUnit }}
                    </div>
                  </div>
        ...
        <!-- Footer -->
        <div
          class="border-t border-gray-200/50 dark:border-gray-700/70 px-4 py-3 flex items-center justify-end"
        >
          <button
