...
//!
//! Uses Natural Earth Admin 0 Countries data (1:50m scale) for continent
//! detection from integer lat/lon tile coordinates (as found in X-Plane DSF
//! filenames, e.g. `+30+135.dsf` → lat=30, lon=135).
//!
//! **Why dilation?**
//! At 1:50m scale the coastlines are simplified.  Some 1°×1° tiles whose
//! centre point falls just offshore return "Unknown" even though the tile
...
//!   path/to/ne_50m_admin_0_countries.zip_or_dir \
...
/// by one 1°-cell into adjacent ocean cells.  5 passes ≈ 555 km at the
/// equator, which covers all coastal airports and narrow straits.
