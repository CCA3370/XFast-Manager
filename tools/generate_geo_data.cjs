#!/usr/bin/env node
/**
 * Generate geo_continent_map.bin from a Natural Earth Admin 0 Sovereignty shapefile.
 *
 * Usage:
 *   node tools/generate_geo_data.cjs <ne_*_admin_0_sovereignty.zip|dir> [output.bin]
 *
 * Default paths (when run from repo root):
 *   input  – ne_*_admin_0_sovereignty.zip (must be supplied as argv[2])
 *   output – src-tauri/src/scenery/geo_continent_map.bin (if argv[3] not specified)
 *
 ...
 *
 * Source data: https://www.naturalearthdata.com/downloads/10m-cultural-vectors/
 *   "Admin 0 – Sovereignty" at 1:10m scale (ne_10m_admin_0_sovereignty)
 *   This dataset includes maritime territories and provides better coverage
 *   for remote islands compared to the standard countries dataset.
 */
...
if (stat.isDirectory()) {
  ...
  // Prioritize sovereignty files first (includes maritime territories), then countries files
  const shpFile = files.find(f => f.toLowerCase() === 'ne_10m_admin_0_sovereignty.shp') ||
                  files.find(f => f.toLowerCase() === 'ne_110m_admin_0_sovereignty.shp') ||
                  files.find(f => f.toLowerCase() === 'ne_10m_admin_0_countries.shp') ||
                  files.find(f => f.toLowerCase() === 'ne_110m_admin_0_countries.shp') ||
                  files.find(f => f.toLowerCase().endsWith('.shp'));
  const dbfFile = files.find(f => f.toLowerCase() === 'ne_10m_admin_0_sovereignty.dbf') ||
                  files.find(f => f.toLowerCase() === 'ne_110m_admin_0_sovereignty.dbf') ||
                  files.find(f => f.toLowerCase() === 'ne_10m_admin_0_countries.dbf') ||
                  files.find(f => f.toLowerCase() === 'ne_110m_admin_0_countries.dbf') ||
                  files.find(f => f.toLowerCase().endsWith('.dbf'));
